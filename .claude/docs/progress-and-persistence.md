...
**CVC-review mode + the firing-order invariant (PR #471, 86c9qa6n3, 2026-06-15).** `pickFocusNode` now returns `{ node, mode: 'forward' | 'cvc-review' }` (App.tsx threads cross-vowel distractor mixing through only when `mode === 'cvc-review'`). The `cvc-review` node comes from `pickCvcReviewNode(progress, sessionCount)` — a graduation session (once, gated by the additive `Progress.cvcGraduationSessionFired` latch) then a 5-session round-robin across the 3 cross-vowel CVC tiers. **THE INVARIANT (a merged regression cost us this — see `testing-and-ci.md` §4.1.8): `pickCvcReviewNode` must be consulted AFTER the forward walk, gated on "no forward progression remains" (the whole `WORD_SONG_NODES_IN_ORDER` tree is mastered) — NEVER before it, gated on `cvcReviewEligible` (only the 3 CVC tiers mastered).** The seductive-but-wrong gate (`cvcReviewEligible` first) hijacks EVERY downstream forward tier: reaching short-i / digraphs / sight-words / simple-sentences _requires_ the 3 CVC tiers mastered, so with the graduation latch still unset (the real forward-learner seed shape) the picker returned the graduation pick `cvc-words-short-u` for every forward session — the wire `focusNode` was wrong for the whole back half of the tree. Review is a standing maintenance layer for the all-done state, never an override of active forward progression.
...
- `chipReadyAtRef` captures the anchor `performance.now()` at the moment chips become tappable. **The capture path split in PR #406** (rationale: `design/research/m4-latency-anchor-decision.md` — automaticity RT runs from cognitive availability, and the old completion anchor recorded fast taps during the read-aloud tail as `-1`, dropping the fastest answerers from the M4.x dataset):
  - **`chipGateVia === 'tts-start'`** (normal path) — captured synchronously inside the chip-gate-open handler when the `read` utterance's `onPlay` fires. This is the real TTS-start moment, when the chips actually open (see `screens-and-flows.md` § Math chip tap-gate).
  - **`chipGateVia === 'fallback'`** (watchdog / speech-error path) — retains the pre-#406 completion anchor: `performance.now()` in a `useLayoutEffect` keyed on `[readAloudPlayed]` (ticket 86c9q5au3 history below). The 2000 ms watchdog-fire timestamp is deliberately NOT used as an anchor — it is not TTS start, and anchoring on it would drift the dataset on silent-audio sessions. In unit tests that open the gate via the watchdog, the recorded value is the `-1` sentinel (genuinely unmeasured), which is expected.
