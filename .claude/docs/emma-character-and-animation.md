...
1. The current SVG assets use a **PNG-in-SVG technique** (rasterised 2000×2000 PNG embedded in an SVG wrapper, ~2.5–3.3 MB each). This drove the bump of `maximumFileSizeToCacheInBytes` from 2 MiB → 4 MiB → 8 MiB (latest lift in ticket 86c9qa7uh) in [vite.config.ts:112](MarianLearning/vite.config.ts#L112). Vector re-trace is tracked separately.
...
- **PNG-in-SVG → vector re-trace.** Asset tickets `86c9kww0z` adjacent. Files are 2.5–3.3 MB each; vector re-trace is the polish-quality target.
- **`attentive-pointing` runtime wiring.** The pose / asset / tilt / spring are all defined, but Math.tsx's hint-state effect doesn't yet `setPose('attentive-pointing')`. Brief §"Missing — what this brief is for" item 5.
- **`listening` runtime wiring.** Same shape — defined but not consumed during caption-reveal beats.
