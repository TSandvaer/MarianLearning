...
#### 4.1.1h Failing-first authors do NOT locally patch production code to prove the green path (W12-05 / PR #410, 2026-06-12)

The RED-proof for a failing-first spec is two-sided: prove RED on base, AND prove the spec _can_ go green. The green side is the **argued mechanism in the PR body** (which upstream change flips which assertion, by what code path) — NOT a temporary local edit to production files that simulates the activator. The auto-mode classifier correctly denied exactly such an edit during W12-05 authoring (the dispatch declared production code out-of-scope), and the wave validated the discipline: the PR-body argument predicted the flip mechanism precisely, and post-merge CI was the GREEN confirmation. A local production patch proves nothing the argument doesn't (the patch isn't the activator's real diff) and risks leaking simulation code into the spec branch.

#### 4.1.1i `math-caption` textContent concatenates words — strip ALL whitespace in caption assertions (Jessica NOF, PR #410, 2026-06-12)

The math caption renders words as CSS-margin-spaced `<span>`s with **no inter-word whitespace text nodes** — `textContent` of the container concatenates them ("Look.One.And..."). Any caption-text assertion must strip ALL whitespace from BOTH sides of the comparison (expected and actual), not just normalize runs of spaces — a normalize-only comparison fails against the zero-space actual. Cost one debug cycle on the W12-05 three-hint spec.
...
#### 4.1.8 A test-helper's convenient default can mask a production-state regression — `pickFocusNode` latch case (PR #471 e2e regression, 86c9qa6n3, 2026-06-15)

A real regression in `pickFocusNode` (CVC-review consulted _before_ the forward walk → every forward-tier session hijacked to `cvc-words-short-u`; see `progress-and-persistence.md` "firing-order invariant") passed the **full vitest suite (3329 green)** yet failed ~30 e2e specs. The vitest blind spot: the picker integration-test helper defaults `cvcGraduationSessionFired: true`, so every existing picker test ran with the graduation latch already SET — which skips the buggy graduation branch entirely. The hijack only fires with the latch FALSY (`cvcGraduationSessionFired: undefined`), which is the **real forward-learner production seed shape** (the latch isn't set until the learner actually reaches graduation). The e2e seeds drive the real production code path, so they carry the falsy latch and caught what the unit tests structurally could not.

**Rule:** when a feature forks on an additive boolean latch (or any field whose test-helper default differs from the fresh-production value), any unit/integration test exercising the not-yet-latched path MUST override the helper's convenient default to the production value — otherwise it silently exercises the wrong branch. A picker test seeding "all-CVC-mastered + a downstream tier in progress" must set `cvcGraduationSessionFired: false`. The fix added 10 such latch-falsy tests, proven RED against the buggy commit. **Corollary:** an APPROVE + green vitest is NOT sufficient when the e2e covers a code path the unit defaults dodge — the Playwright gate is load-bearing here, not redundant (it's why we never merge past a red e2e even on an approved PR).
...
**Second footgun — `git checkout --ours/--theirs <file>` takes the WHOLE file, silently reverting clean auto-merged hunks (PR #409 rebase, 2026-06-12).** During a rebase conflict where most of a large file auto-merged cleanly and only one hunk conflicted, `git checkout --theirs <file>` replaces the ENTIRE file with the feature-branch version — discarding every auto-merged hunk from the target (in the #409 case, silently reverting the just-merged PR #406 latency anchor from Math.tsx; caught only because a regression test covering the reverted behavior went red). **Rule: when most of a file auto-merged, resolve the conflicted hunk(s) manually in the editor — never whole-file `--ours`/`--theirs`.** Whole-file flags are safe only when the file genuinely has a single owner side (e.g. a coordination log, or a file one branch deleted).
