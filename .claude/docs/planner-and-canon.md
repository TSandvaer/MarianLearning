...
`effectiveFocusNode` ([\_planner.ts:539](MarianLearning/api/_planner.ts#L539)) — for word-song, valid-but-untuned nodes (`letter-sounds`, `digraphs`, `sight-words`, `simple-sentences`) fall back to `blending-cv` content as a stub. Math honours caller-supplied focusNode verbatim. Cross-track focus nodes are hard-rejected upstream by `generateSessionPlan` ([\_planner.ts:271](MarianLearning/api/_planner.ts#L271)).
