...
**A re-bake re-draws problem content — splice, don't re-bake, when reconciling a canon rebase (PRs #459/#460, 2026-06-15).** Two branches that each re-bake the same word-song tier produce **byte-identical deterministic lines** (recap / streak / opener / giveAnswer-template — fixed text+voice) but **different problem content** — Haiku re-draws `plan.id`/`label` + the p2-p8 read/correct/hint/giveAnswer words on every bake. So when two branches both touched a tier's single-line canon JSON (e.g. #459 added a `blend` slot, #460 added `session.end.recap.focus`) and you rebase one onto the other, a wholesale "take one side" conflict resolution **silently churns problem content**. The byte-safe reconciliation for "add slot X onto main's tier" is: resolve to **main's content** + **render ONLY the new slot** and splice it in (blend / letter-sounds audio is deterministic from `(word, tier)` via `renderSessionAudio` — no Haiku call needed), then byte-verify nothing else moved. **Gotcha:** `.env.local` values are quote-wrapped (`AZURE_SPEECH_REGION="westeurope"`) and `generateSessionCanon`'s loader strips them — a standalone bake script that reads `.env.local` must replicate the quote-strip or every Azure fetch fails as "fetch failed" (not an auth error).
...
**Isolated stop consonants scratch in the CVC phoneme-blend render (voice-QA #463, blend feature #459 — resolution pending).** `renderBlendInnerText` ([\_tts.ts:1183](MarianLearning/api/_tts.ts#L1183)) voices the `blend` slot ("c - a - t ... cat") by wrapping EACH grapheme in a bare IPA `<phoneme>` (consonant → bare IPA: c→k, b→b, g→ɡ, …) with a `<break time="250ms"/>` before it, the whole line at `<prosody rate="-12%">`. **Azure neural TTS cannot cleanly articulate an isolated stop consonant** (/k/, /b/, /p/, /g/, /d/, /t/, /f/, /v/) with no vowel to lean on — it renders as a _scratch_. Confirmed empirically by sponsor ear-test: voice-QA report #463 failed ~37 of 40 blend clips, every note the same shape ("scratch no C, then AT then CAT"); the vowels (æ/ɒ/e) and the natural whole-word sustain fine. The render reuses the letter-sounds tier's `<phoneme>` machinery but stacks the `<break>`-before-each-phoneme + `-12%` envelope on top — an A/B audition page (PR pending) is isolating which lever aggravates the scratch. **Takeaway for any future "sound it out phoneme-by-phoneme" feature: bare isolated stops via `<phoneme>` are unreliable in Azure neural TTS — audition candidate renders against the sponsor's ear before baking at scale, and never add a schwa-release ("kuh") to dodge it (synthetic-phonics anti-pattern).**
...
#### Nested `<prosody>` compounds multiplicatively, it does NOT override (blend pass-3 audition, PR #472, 2026-06-16)

An inner per-segment `<prosody rate>` / `<prosody pitch>` nested **inside** the speak-root prosody envelope COMPOUNDS with the outer setting — it does not override it. Measured empirically on `en-GB-OliviaNeural` with identical inner text (`"ef"`): outer `-10%` only → 9936 bytes; flat `-20%` only → 11088 bytes; nested `-10%`(outer) × `-20%`(inner) → 12384 bytes. The nested clip being **longer than the flat `-20%`** is the decisive signal — a true override would make `nested == flat-(-20%)`. Pitch offsets layer the same way. Implication for any per-segment prosody in `api/_tts.ts` (e.g. the per-phoneme onset wrappers in the blend render): the inner rate/pitch applies on top of the outer envelope, so compute the net effect — no flatten workaround is needed to make the inner setting "take".

- **`"fetch failed"` ≠ Azure SSML rejection.** In the render scripts (`renderBlendAudition.ts`, and the same `fetch()`-to-Azure shape in `_tts.ts`), a structural markup problem surfaces as `Azure <4xx>: <body hint>`; a bare `fetch failed` is a transient network/connection error from `fetch()` itself, NOT a markup rejection. `renderBlendAudition.ts` records per-candidate failures but does **not** retry and overwrites the manifest wholesale, so a clean **re-run** is the recovery (a tail cluster of 7 transient `fetch failed` cleared on a plain re-run; same SSML shapes had succeeded earlier in the run). Retry before treating a `fetch failed` as a broken-markup bug — cf. the `feedback_retry_before_revert` memory.

#### Runtime Azure render path + debug-endpoint feasibility (blend-tweak handle, PR #473, 2026-06-16)

- **TTS renders at RUNTIME, not only at build-time.** Beyond the build-time canon prebake, the session-start path renders audio live: `api/claude.ts` → `api/_session.ts` → `synthesizeUtterance` calls Azure on a cache miss. Consequence: Azure creds are present in the Vercel **runtime** env on **every PR preview** (if they weren't, every preview Word Song / Math session would 502 `tts-failed`). So a live "type params → hear it" debug render endpoint works on a PR preview with no extra env wiring — confirmed empirically by hitting `/api/blend-tweak` on the preview and getting a real MP3 back.
- **Gate debug render endpoints with `VERCEL_ENV`.** It's `'production'` only on prod, `'preview'` on PR previews, `'development'` locally. A debug render route should `return 404` when `VERCEL_ENV === 'production'` (404 not 403 makes the route look non-existent on prod). Pair with the existing per-IP rate limiter (`api/_rateLimit.ts`) and `escapeSsml` on any sponsor/free-text input.
- **To push pre-assembled SSML through the production Azure path, use the `ssmlOverride` seam on `synthesizeUtterance`** (added in #473, pending merge) rather than letting `buildSsmlBody` re-run `renderSsmlInnerText` on already-escaped markup (which double-escapes). Reusing the production `BLEND_GRAPHEME_IPA` / `BLEND_STOP_GRAPHEMES` constants for the medial + coda means a tuned onset is auditioned against byte-identical production context, so a winning onset transfers directly into `renderBlendInnerText`.
- **CAVEAT — the preview's Azure REGION can differ from local and reject IPA features the bake pipeline accepts.** The live `/api/blend-tweak` endpoint returned 200 for text + bare IPA but **400 for length-mark / schwa IPA** (`fː`, `sː`, `kə`) on the Vercel preview, while byte-identical SSML rendered 200 against local `westeurope`. The preview's `AZURE_SPEECH_REGION` en-GB-Olivia deployment rejects IPA that westeurope honours. So **"preview has Azure creds" ≠ "preview renders everything local does."** To audition **IPA-dependent** audio (length marks, phoneme overrides) reliably on a preview, **bake STATIC clips through the local/westeurope pipeline** (base64 in the data JSON → region-independent, plays on any preview) instead of leaning on the live runtime endpoint. The length-mark lever itself (`fː`/`sː` as a held fricative) is real — it just can't be exercised via the preview's runtime region. (blend pass-3/4, PR #472/#473, 2026-06-16.)
- **Blend (and anything `renderBlendInnerText` emits) is RUNTIME-reachable — NOT pre-baked-canon-only.** Graduation `cvc-words` sessions set `bypassCanonAndCache` (`api/claude.ts:855`) and render the session audio LIVE: the Haiku planner emits the blend slot for every cvc-words problem (`api/_planner.ts:3296`) and it routes through `renderBlendInnerText` at runtime (`tierFilter='cvc-words'` ∈ `BLEND_CVC_TIERS`). So any SSML feature added to the blend render (e.g. an IPA length mark) is subject to the **production RUNTIME** Azure region's capabilities, not just the build-time bake region. The failure mode is NOT a 502 — `api/_session.ts:199-220` soft-fails per-utterance (ticket 86c9kjdh2): a region-rejected clip is logged + SKIPPED, returning 200 with that one clip **silently absent** (browser falls back to the silent caption-walk). So a build-time-only "does this SSML render?" check is insufficient for blend changes — verify the runtime region accepts it too, or gate the feature to the bake path. (Traced during the blend pass-5 production gate, 2026-06-16.)

#### Blend pass-5 — runtime-safe render + bake-vs-runtime fidelity split (PR #476, 2026-06-17)
...
- **Full-fidelity (opt-in, BAKE only):** the per-class segmented render — stops `kə`; `/h/`=`hə` (fric-rel); `/f/`=`<prosody rate="-25%"><phoneme ph="fːə">f</phoneme></prosody>`; `/s/`=`sːə`; `/v/`,`/dʒ/`(grapheme `j`),`/w/`=whole-word FLOOR (`BLEND_FLOOR_GRAPHEMES`/`wordIsFloored()`). The bake path (`generateSessionCanon.ts::bakeOne`, `rebakeBlendCandidateF.ts`) sets `blendFullFidelity: true`; the runtime path (`api/claude.ts`) passes no renderOptions → safe default.
...
**Byte-preservation of a per-class re-bake is PER-WORD, not blanket.** A full-fidelity re-bake only changes clips whose SSML actually diverges from the prior bake (pass-5: 15 of 40 — the f/s/h/v/j/w words); stop/continuant/vowel-only words re-render byte-identical (their full-fidelity SSML == candidate-f). `verifyBlendBytePreservation.ts` derives the expected-change set from grapheme class — a blanket "all N must change" assertion false-FAILs on a per-class fidelity bump. The fricative treatment is keyed per-**grapheme**, not per-onset: `bus` (b-u-s) gets `sːə` on its CODA `s` and diverges even though its onset is the stop `b` — any "which words change" reasoning must scan ALL graphemes, not just the initial one.

**Blend `tierFilter` routing is load-bearing + fails silently.** `renderBlendInnerText` only fires when `tierFilter ∈ BLEND_CVC_TIERS`. If a CVC tier's focus node isn't in `WORD_SONG_FIRST_CLASS_FOCUS_NODES`, the bake routes it to `blending-cv`, `tierFilter` mismatches `BLEND_CVC_TIERS`, and `renderBlendInnerText` early-returns null → blend audio silently falls to the plain (non-segmented) path with NO error. Worth adding to `sibling-tier-checklist.md` when a new CVC tier ships.
