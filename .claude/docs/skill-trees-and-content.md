...
#### `DISTRACTOR_ONLY_WORDS` — repopulated; the renderable-gap-target `TARGET_PAIRINGS` invariant

PR #208 (short-e canon-wire, ticket 86c9teua2) once _emptied_ `DISTRACTOR_ONLY_WORDS` by promoting its last member (`pen`) to `TARGET_WORDS` — but it has since been **REPOPULATED**: the digraphs work added entries like `sell`, and the cat-sat-mat #429 work added the gap verb `bit`. The array is **NOT empty** — do not assume it is. (Verified against `wordPack.ts` 2026-06-14: `DISTRACTOR_ONLY_WORDS` opens with the `sell` … `bit` block ~L1509.)

**The export is intentionally retained** regardless of population — `wordPack.ts` keeps `export const DISTRACTOR_ONLY_WORDS: readonly WordEntry[]` so (1) future tiers have a typed home for distractor-only pictures, and (2) `ALL_WORDS = [...TARGET_WORDS, ...DISTRACTOR_ONLY_WORDS]` stays expression-stable and `getWordEntry` callers don't care which array a word came from.

**⚠️ Renderable-gap-target invariant (the #439 `bit`-throw bug, ticket `86ca8jdt6`).** A word can be `isTarget: false` in `DISTRACTOR_ONLY_WORDS` yet still be **rendered as a gap target** when it is a member of `SIMPLE_SENTENCE_TARGET_SET`: the simple-sentence render path (`WordSong.tsx` `buildChipOrder` → `pickDistractors(target)`) hits the matrix **unconditionally**, so such a word MUST carry a `TARGET_PAIRINGS` row or it throws `no pairing matrix entry` at mount. The exhaustiveness scan in `wordDistractors.test.ts` only iterates `TARGET_WORDS`, so it does **not** catch a missing row for a `DISTRACTOR_ONLY_WORDS` member — exactly the blind spot that let `bit` ship rowless and crash the "The cat \_\_\_ the bag." (#429) plan. **Rule: every member of `SIMPLE_SENTENCE_TARGET_SET` needs a `TARGET_PAIRINGS` row, independent of `isTarget`.** (The `wordPack.ts` comment near the `bit` entry claiming simple-sentence foils "come from the planner-side distractor table, never from `TARGET_PAIRINGS`" is FALSE — no such planner foil table exists; correction tracked as `86ca8n7mm`. #439 adds a `SIMPLE_SENTENCE_TARGET_SET` exhaustiveness test — pending merge.)
