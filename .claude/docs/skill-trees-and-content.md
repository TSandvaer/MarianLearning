...
- **sceneId frame-collision across gentle/trap phases.** `"The dog ran ___."` normalises identically whether it's a gentle row (gap `in`) or a trap row (gap `there`); deriving `sceneId` from the frame alone attaches the gentle scene to the trap problem. The fix gates on the gentle window (problem index ≤ 3) — phase isn't on the wire but index is. `normalizeSentenceFrame` (`sceneRegistry.ts`) is intentionally NOT a unique key. Scene assets live at `public/assets/scenes/scene-<sentence-id>.svg`; `SCENE_PICTURES` (`scenePictures.tsx`) is empty until Thomas's MJ pack lands, with graceful text-only fallback.
