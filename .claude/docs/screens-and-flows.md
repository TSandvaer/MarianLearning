...
**ACs touched** (high level — full ACs in the spec): chip taps gated until the `read` utterance's TTS **starts** (see chip tap-gate below); per-correct stardust grant + sparkle burst; advance after 1.2 s on correct (or hard-ceiling 1.6 s); guided pose after 2nd wrong (`GUIDED_AFTER_WRONG_COUNT`), give-answer after 3rd; surface tag `'math'` flows through to SessionEnd via the backwards-compat shim.

**Chip tap-gate — opens on TTS START, not completion (PRs #399 / #402 / #406).** Answer chips are held closed by `chipGateOpen` state until the `read` utterance's `onPlay` fires (`chipGateVia = 'tts-start'`). DOM attributes on the chip container: `data-chip-gate` (`'open'`/`'closed'`) and `data-chip-gate-via` (`'tts-start'`/`'fallback'`) — these are the e2e seams. Two fail-open paths prevent soft-lock: a `CHIP_GATE_FALLBACK_MS = 2000` watchdog (silent audio, suspended context) and an immediate open on `reportSpeechError`. Only the `read` utterance drives the gate — `speak(text, isReadAloud)` threads the flag; correct/reprompt/hint/giveAnswer `onPlay` events never touch it. `readAloudPlayed` **deliberately retains completion semantics** (it anchors `data-read-aloud-played` + ~20 cold-mount/audio-race regression tests — the separation is a PR #402 design decision, do NOT unify). The chip `disabled` prop is overloaded (`resolved || dimForGuided || !readAloudPlayed`) — specs key the gate dimension on `data-chip-gate`, never `disabled`. Spec home: `design/screen-3-math.md` § "Thinking-time chip tap-gate (gated on TTS START)"; pedagogy: Dave's MODIFY ruling on ticket 86ca7urvk (TTS-start not a 500 ms timer, no parent-settings knob, Math-only). Auto-advance is independent of this gate — it fires on the CELEBRATION utterance's `onend` (`correctSpeakResolvedRef`) or `ADVANCE_HARD_CEILING_MS = 4000`.
...
| `latencyMsByProblemRef`        | `number` (ms; `-1` sentinel for not-measured)                 | `chipReadyAtRef` anchor: gate-open handler when `chipGateVia === 'tts-start'`; `useLayoutEffect([readAloudPlayed])` completion path when `'fallback'` (PR #406 split) + chip-tap delta | M4 Leitner / M4.x slow-fact directive                                    |
