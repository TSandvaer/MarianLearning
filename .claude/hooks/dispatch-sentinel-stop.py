...
Per [[feedback_agent_staleness]] every Agent({run_in_background: true}) must
be paired with a ScheduleWakeup (or one-shot CronCreate recurring=false) at
~2x expected duration so a stale agent gets noticed even during active
conversation. The 2026-05-13 (2 Kevin agents) and 2026-05-15 (Jessica
forceHowlerUnlock) incidents proved the behavioral rule alone is insufficient.
...
def main():
    ...
    if agent_bg_count > 0 and tripwire_count == 0:
        reason = (
            ...
            "tripwire. Per [[feedback_agent_staleness]] every "
            ...
            "during active conversation. The 2026-05-13 + 2026-05-15 "
            "incidents proved the behavioral rule alone is insufficient; "
            ...
            "SendMessage-ping any silent agent (per "
            "[[feedback_no_idle_no_stale_agents]] rules 2 + 3)."
