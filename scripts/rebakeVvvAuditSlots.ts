 ...
 * vv2 render (held + schwa-tail `vːə` @ rate -35%) into the existing
 ...
 *   - The vv2 change (api/_tts.ts PHONEME_OVERRIDES.vvv + SCRATCHY_PROSODY_BY_
 *     MNEMONIC.vvv) ONLY affects clips whose text contains the `vvv` mnemonic.
 *     The only canon carrying `vvv` mnemonic text is `letter-sounds-audit.json`
 *     at `word.p2.{read,correct,hint,giveAnswer}` (the shipped letter-sounds.json
 ...
 * clip (incl. the shared `word.p2.reprompt` "Hmm... try again?") is preserved
 * byte-for-byte.
 *
...
async function main(): Promise<void> {
  ...
  console.log(
    `\nDone. Re-baked 4 /v/ audit slots (vv2: held vːə @ rate -35%). ` +
      `Every other clip preserved byte-for-byte. Voice = EMMA_VOICE_CONFIG (Olivia).`,
  )
}
