...
- sub-to-10: subtraction with both operands in 0-10 and answer in 0-10. read: "<minuend> minus <subtrahend>. How many are left?" e.g. "Seven minus three. How many are left?"
  ...
  PER-PROBLEM SHAPE for sub-to-10: every problem MUST emit op: "-" on the wire (the screen renders the operator glyph from op). Wrong-answer chip selection (the "off-by-one" vs "wrong-op trap" choice for P4-P8) is handled entirely at render time in src/screens/Math/Math.tsx and is NOT part of the planner output — emit only the fields listed below. Utterance ids MUST use the literal "math." prefix (NOT "sub-to-10."): "math.p1.read", "math.p1.correct", ..., "math.p8.giveAnswer". The id namespace is the track name, NOT the focus-node name. Per-slot utterance templates:
