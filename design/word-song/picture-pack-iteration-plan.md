...
- **Never reference IP / brand names in `--no`.** Cartoon-character and branded-toy names trip moderation even when negated. Rely on positive wording + generic anti-anthro instead. (A branded spinning-top name once slipped into the PR #189 `top` prompt as a shape-disambiguator; it has since been scrubbed to the generic `plastic spinning disc top` descriptor, which reads cleanly and carries the same shape-guard intent without naming a brand.)
