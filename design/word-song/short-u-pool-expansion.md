...
| --- | ---- | ----------------------------------------------------- | ----- | --------- | --------------------------------------------------------------------------------- |
| 1   | sun  | EXISTS as distractor; re-trace per pack-cohesion pass | u     | celestial | Promoted from distractor → target.                                                |
| 2   | cup  | EXISTS as distractor; re-trace per pack-cohesion pass | u     | vessel    | Promoted from distractor → target.                                                |
...
| 11  | gum  | NEW                                                   | u     | food      | Wrapped chewing-gum stick. **Q1 in §10** — drop or keep based on Thomas review.   |
...
| --- | ---- | --------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | sun  | EXISTS as distractor; re-trace optional in v1 | `public/assets/pictures/picture-sun.svg` | Existing file is a hand-authored SVG from the short-a pack ship (PR #157). Optional re-trace via Phase 3 PNG-embed pass for stylistic cohesion with the 8 wholly-new short-u additions. **Defer to Thomas's preference** — Q2 in §10. |
| 2   | cup  | EXISTS as distractor; re-trace optional in v1 | `public/assets/pictures/picture-cup.svg` | Same rationale as `sun`.                                                                                                                                                                                                              |
...
| 11  | gum  | NEW (conditional on Q1)                       | `public/assets/pictures/picture-gum.svg` | Single wrapped chewing-gum stick — wrapper opened halfway to show stick — OR a single wrapped flat package. **If Q1 drops `gum`, this row drops.**                                                                                    |

**Total new picture-pack PNG generations needed: 8** (`bug, nut, tub, bun, jug, rug, hut, gum`). If Q1 drops `gum`, drop to 7. If Q2 says re-trace `sun/cup/bus`, add 3 → 10–11 total.
...
- Short-u pack: 8 new (or 7 if Q1 drops `gum`) + 0–3 re-trace (sun, cup, bus, depending on Q2)
- **New on this PR pack: 8 (or 7).** Cumulative: ~34 picture-pack SVGs after this ships.
...
### Pipeline for the 8 short-u pictures
...
**Q1. Pool size — 11 vs. 14 vs. drop `gum`.** The audit rigorously yields 11 strong entries (sun, cup, bus, bug, nut, tub, bun, jug, rug, hut, gum). The brief asked for ~14. Forcing 14 drags in weaker entries (`pup` silhouette-collision, `mug` cup-collision, `mud` mass-noun-instability, `pug` collision). Forcing the cleanest possible pool drops `gum` (mass-noun) for 10 strong entries.

Three options:

- **A. Ship 11** (recommended) — strongest pool that still has `/ʌm/` representation via `gum`. `gum`'s wrapped-stick picture mitigates the mass-noun risk.
- **B. Ship 10** — drop `gum`, accept that `/ʌm/` has no representation. Cleaner but coverage-narrower.
- **C. Ship 14 with weaker entries** — adds `pup`, `mug`, `mud` despite audit defects. Not recommended.

**Decision:** Recommend A (11). Thomas — confirm or pick B/C.

**Q2. Re-trace `sun`/`cup`/`bus` for cohesion?** These three already have shipped picture-pack SVGs from PR #157. Should Phase 2 re-generate them via the new MJ session for cross-pack stylistic cohesion (as short-o spec §3 recommended doing for `dog`)?

Two options:

- **A. Re-trace** — adds 3 generations to Phase 2 for cohesion. Aligns with the short-o pack's recommendation for `dog`.
- **B. Defer** — keep existing assets; re-trace only if Phase 2 review shows visible style mismatch on iPad.
