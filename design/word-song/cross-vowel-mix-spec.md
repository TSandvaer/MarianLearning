...
**Q3. Should cross-vowel mode ship with a one-time Emma "we're mixing vowels now" first-session opener?** **OPEN — recommend NO for v1.** Mirrors short-o `box`/`fox` and short-u `/u/` vs `/ʌ/` first-encounter scaffolding patterns. Argument FOR adding: makes the moment legible to Marian, aligns with Emma's existing "warm, observant teacher" voice. Argument AGAINST: Marian arrives at cross-vowel mode having mastered three vowels — she has the receptive vocabulary and the discrimination skills are ready; an explanatory line risks reading as "this is harder" cue that primes her for friction rather than letting her experience the natural progression. Default recommendation: **skip the opener for v1 ship.** If Thomas wants it added, Kyle drafts the line under a follow-up ticket and bakes into the cross-vowel-active session-start utterances. Cost-of-default-skip = Marian gets cross-vowel chips with no fanfare, just decoded normally. Cost-of-add-now = one extra session-start utterance to author + canon-bake. Skip-default; add later if signal warrants. **THOMAS CONFIRMS / OVERRIDES.**

**Q4. Cross-vowel discrimination-readiness research follow-on.** **OPEN — recommend FILE.** Per §3, Dave's existing research is partially silent on the precise threshold question (per-aggregate-mastery sufficient, OR additionally requires per-vowel novel-pool probes?). v1 ships with per-aggregate-mastery as the threshold; if the research deliverable later says "actually, cross-vowel work needs novel-pool generalization on each vowel," the gate tightens via one predicate constant change in `mastery.ts` — additive, non-breaking. **Recommendation: Matt files a research ticket for Dave** with the v2-of-this-spec scope. Cost-of-filing-now = one ticket in the backlog. Cost-of-not-filing = future-Kyle re-derives the threshold question when v2 lands. File. **THOMAS CONFIRMS / OVERRIDES.**

All four questions are scoped for Thomas's review; Q1 and Q2 carry strong recommendations with cost-asymmetry rationale. Q3 and Q4 are genuine asks. AC items above reflect the Q1/Q2 locks (default-ON cross-vowel; per-aggregate gate).
