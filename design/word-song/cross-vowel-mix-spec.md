...
`design/research/phonics-sequence-marian.md` §Q1 (revised vowel order locked 2026-04-26) sources the importance of short-vowel discrimination work but does NOT explicitly time when _cross-vowel chip discrimination_ (as a session-design feature) becomes appropriate. The closest evidence is Source 4 (Reading Universe / Really Great Reading on ELL short vowels) at lines 57–60 of the phonics doc:

> Recommends illustrated word pairs with contrasting vowel sounds and explicit meaning-instruction alongside phoneme work.

That language (illustrated word pairs with contrasting vowel sounds) is a structured-literacy primitive — and it matches what cross-vowel mixing surfaces inside the chip trio (target with vowel V1, distractor with vowel V2). So the principle is endorsed; the _timing_ (post-consolidation of multiple same-vowel tiers) is consistent with the §Application short-vowel introduction sequence at lines 159–171, but is not directly stated.

Source 13 (Brookes / Cardenas-Hagan, lines 117–120) on ELL pacing also doesn't directly address cross-vowel chip-discrimination timing; it speaks to pacing of new pattern _introduction_, not pacing of _contrastive trial design_.

### Gap — recommended follow-on research ticket

**Dave's research is partially silent on the precise cross-vowel-discrimination timing question.** §2 of his developmental review supports the _concept_ but doesn't pin the _threshold_ (e.g., does Marian need 90/3 mastery on each of the 3 vowels before cross-vowel? Does she need novel-pool generalization on each? Or is multi-tier mastery the right gate?).

**Recommendation: file a follow-on research ticket** for Dave (or whoever owns the next phonics-research deliverable) — _"Cross-vowel discrimination-task readiness: when does the mastery threshold per vowel meet the threshold for vowel-contrast chip work?"_ The output should clarify whether per-aggregate mastery is sufficient (Option A here, the locked default) OR whether each consolidated vowel additionally needs a novel-pool generalization probe (mirroring `cvc-words` §"Graduation gate" behaviour). Until that research lands, this v1 spec uses **per-aggregate mastery** as the safest no-extra-gate posture; the gate can tighten later without breaking the matrix shape.

Suggested ticket title: _"Research: cross-vowel discrimination readiness threshold (consult to ticket `86c9m3aek` v2)."_ Matt to file under `research/` with Dave as owner.
...
1. **Vowel mix rule.** Each row's TWO distractors should EACH carry a _different_ vowel from the target. So a short-a target's gentle/trap distractors are EITHER `[short-o, short-u]` OR `[short-o, short-o]` OR `[short-u, short-u]` etc. — _not_ both same-vowel-as-target. (At least one cross-vowel chip; ideally both.) For trap-tier specifically, prefer a _vowel-contrast_ pair (`[short-o, short-u]`) because that maximises the discriminative pressure.
...
This example is preserved with the working-out visible because it shows where the matrix authoring genuinely has tradeoffs. The /ʌn/ rhyme family doesn't extend to short-o in the v1 pool, so the textbook minimal-triplet is unavailable; the matrix author has to settle for partial-rhyme + vowel-contrast. **Kevin's impl ticket inherits this constraint** — for some short-u targets, the trap-tier cross-vowel pair won't be a perfect minimal triplet. That's fine; rule (1)'s preference for vowel-contrast pair is a _preference_, not a hard rule. Where rule (1) and a clean minimal-triplet conflict, prefer the minimal-triplet's pedagogical clarity over forced cross-vowel-pair purity (i.e., a `[fan, can]` trap for `sun` would be _both short-a_ but is a textbook /n/-coda discrimination drill — acceptable as a fallback).
...
These gaps are acceptable. The matrix doesn't need exhaustive minimal triplets to do its work; it needs _enough_ contrastive pairs across the 33 targets that every gentle and trap row is well-formed against the constraints. Kevin's authoring exercise will reveal where the pack genuinely runs out — at that point a follow-up ticket can extend the picture pack (e.g., adding `cot` would unlock several minimal triplets), but it's not blocking for v1 cross-vowel mixing to ship.
...
**Q5. The firing layer — how does cross-vowel mixing actually surface in regular play? (CLOSE-OUT, ticket `86c9qa6n3`).** **RESOLVED 2026-06-15 — mechanic locked Option C-then-B per Dave's research (`design/research/cvc-review-mode-mechanic.md`), implemented in ticket `86c9qa6n3`.**

This spec (`86c9m3aek`) shipped the cross-vowel matrix + `crossVowelMixingActive` predicate + parent toggle as **forward-compatible infrastructure**. Kevin flagged the resulting paradox in PR #181's Self-Test Report: once all three CVC tiers reach `'mastered'`, `crossVowelMixingActive` returns `true` — but `pickFocusNode` walks PAST every mastered CVC tier onto a non-CVC node (`digraphs-sh`), so `App.tsx`'s `focusIsCvcTier` gate never threads the mode in. Net: zero cross-vowel chips surfaced in regular v1 play. Thomas accepted the forward-compat posture on 2026-05-09 and filed the firing-layer follow-up (`86c9qa6n3`).

**The locked firing mechanic (Option C-then-B):**

- **Graduation review (Option C — once).** The first session after all three CVC tiers are mastered fires a one-shot celebratory cross-vowel session on `cvc-words-short-u`. Short-u is chosen because `/ʌ/` has no Tagalog equivalent and is Marian's highest-L1-interference vowel (per `cross-vowel-discrimination-threshold.md`). Gated by an additive `Progress.cvcGraduationSessionFired?: boolean` latch — set at the session-end write so the graduation review never repeats.
- **Periodic revisit (Option B — round-robin).** Thereafter, every 5th completed session re-surfaces a mastered CVC tier on a round-robin across the three tiers (`CVC_TIERS[Math.floor(sessionCount / 5) % 3]`). This maintains phoneme-boundary sharpness against the forgetting curve (Rosenshine Principle 1 + Cepeda spacing).
- **Option A (Leitner-for-words) rejected** — phonics discrimination is procedural memory (automaticity), not declarative recall; per-word promote/demote misframes the learning target.

**Implementation surface (`86c9qa6n3`):** new `pickCvcReviewNode(progress, sessionCount): WordSongNode | null` in `focusNode.ts`; `pickFocusNode` return widened to `{ node, mode: 'forward' | 'cvc-review' }`; `App.tsx`'s `focusIsCvcTier` gate lets the mode through when `focusMode === 'cvc-review'`; `cvcReviewEligible(progress)` predicate in `mastery.ts`; the `cvcGraduationSessionFired` latch (additive, no schema bump; read-path defaulter + cloudSync parity). The cross-vowel matrix + predicate + toggle from this spec are unchanged — `86c9qa6n3` is purely the picker-side firing layer that consumes them.

**Periodic-review `skillFocus` recording — fixed in `86ca9atqh` (was a known limitation of `86c9qa6n3`).** The PERIODIC review branch of `pickCvcReviewNode` is `sessionCount`-dependent (`sessionCount > 0 && sessionCount % 5 === 0`, round-robin index `floor(sessionCount / 5) % 3`). At session-start, `App.tsx` passes the real `sessionCount` (from the `marian-tutor.session-history.v1` blob) into `pickFocusNode`, so the session genuinely runs as a cross-vowel review on the round-robin-selected CVC tier. But `SessionEnd`'s mount-effect originally **re-derived** the focus via `pickFocusNode(progress, track)` with `sessionCount` **omitted** (→ 0) for the P0.2 `skillFocus` write — and a sessionCount-blind re-derivation falls through to the forward walk, landing on the next non-mastered node (e.g. `digraphs-sh`). That mislabel was NOT inert: `applyMasteryRule`'s `qualifies()` filters history by `entry.skillFocus.includes(node)` (`mastery.ts:620`), so a high-scoring periodic cross-vowel review wrongly credited the forward tier's 90/3 promotion counter — a **mastery-contamination correctness bug**, not a cosmetic history label. Over enough interleaved 5th-session reviews, Marian could be promoted past a phonics tier she never actually decoded. (The one-shot GRADUATION review was always correct — its branch is `sessionCount`-independent, so the kick-effect and the SessionEnd re-derivation agree.) **The fix (`86ca9atqh`)** threads the picked session focus IDENTITY (`{ node, mode }`) — frozen at session-start kick-time from the same sessionCount-aware `pickFocusNode` call that drove the `/api/claude` request — through the `SessionEndPayload.sessionFocus` field, so `SessionEnd` records the ACTUAL focus the session ran under instead of re-deriving it sessionCount-blind. SessionEnd retains the sessionCount-blind re-derivation only as a back-compat fallback for callers that don't ship the field (hand-built test fixtures); that fallback stays correct for every branch except the periodic review, which is precisely the branch the threaded field exists to fix.

All five questions are now locked. Q1 + Q2 carried strong recommendations with cost-asymmetry rationale; Q3 + Q4 were genuine asks resolved by Thomas on 2026-05-09; Q5 is the firing-layer close-out resolved by the `86c9qa6n3` implementation. AC items in §7 reflect Q1/Q2 locks (default-ON cross-vowel; per-aggregate gate); Q3 lock is a no-op on AC; Q4 lock surfaces a Matt-driven follow-on ticket but no AC change in this spec; Q5 is documentation of the downstream firing ticket, no AC change here.
