...
**Q1. Pool size — 9 vs. drop `egg` to 8 vs. force a 10th.** **LOCKED 2026-05-09 by Thomas (Option A — ship 9 audit-derived).** Pool: `bed, leg, hen, pen, web, net, jet, gem, egg`. `egg` is retained under the 3-letter-spelling-CVC precedent from short-o `box`/`fox`. Phase 2 fallback to 8 (drop `egg`) stays documented as the contingency if Thomas finds the smooth-ovoid read unstable at 96pt during MJ review. AC items in §9 reflect the 9-word ship. The §3 prompt sheet ships 9 (8 wholly-new + 1 conditional `pen` retrace per Q2).
...
**LOCKED 2026-05-09 by Thomas (Option B — short-e spec lands now; impl gated on short-i shipping first).** Natural arc: short-i pool spec PR #180 + sibling impl PR ship before short-e impl PR is dispatched. The §5 2-session-gap predicate's precondition (`cvc-words-short-i === 'mastered'`) is satisfied because Marian goes through short-i mastery before short-e is even unlockable. Each tier gets independent review + e2e cycle. Matt's ticket sequencing reflects this lock — short-e impl ticket carries an explicit "blocked on short-i tier shipping" header.
