...
**Q3. Phase 2 fallback policy — substitute `cob` automatically, or escalate?** **LOCKED 2026-05-09 by orchestrator on Thomas's behalf (Option A — automatic substitution).** Per cost-asymmetry: round-trip cost of escalation outweighs marginal vocab-quality difference; lollipop and corn-cob are both vocab-stretch picture-grounded entries, both audit-acceptable; the standby `cob` prompt already lives in `short-o-picture-pack-extension-prompts.md`. If Phase 2 review of `pop` lollipop fails 96pt, Devon swaps in `cob` without escalation; pool stays at 11; spec is amended in a follow-up PR to reflect the substitution. Cross-vowel-mode pool-size floor (≥ 11) is preserved either way.
