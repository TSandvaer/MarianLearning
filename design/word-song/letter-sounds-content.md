...
The genuinely-new A5 decisions are: (i) read-line template phrasing for an isolated-phoneme prompt (resolved inline §2.1), (ii) SSML scope for phoneme utterances (resolved inline §2.3 + §2.4 — substitution-table extension of `PHONEME_OVERRIDES`, see §8 appendix), (iii) per-session vowel-progression rule (resolved inline §1.4), (iv) /i//e/ adjacency ban enforcement in-session vs. cross-session (resolved inline §1.2 + §1.4), and (v) the mastery-progression decision for whether `letter-sounds` masters per-vowel or as one composite tier (was sponsor Q4 — RESOLVED 2026-05-23 to Option B composite for Wave 7, with Option A per-vowel as the deferred follow-up; **Wave 9 is that follow-up — see `design/wave-9-plan.md` — and Option A is now the SHIP target with Option B retained as the field-absent fallback path; §5.3 + §6 reflect Option A as the Wave 9 ship**) + the mastery-threshold calibration (was sponsor Q6 — RESOLVED 2026-05-23 to standard word-song 90/3, no per-tier override). Originally only (v) escalated to §7; both Q4 and Q6 in (v) are now resolved via team consultation per `[[feedback_no_sponsor_as_expert]]` and the §7 entries are kept as resolved-records for audit trail. See §8 appendix for the routing audit on review-resolved items.
...
2. **Cross-session, `/e/` introduction MUST wait until `/i/` has reached `mastered` status — enforced as a runtime predicate, NOT a directive approximation.** The hard gate is `letterSoundsVowelStates['/i/'] === 'mastered'`: the planner refuses to introduce `/e/` as current-target until that predicate holds. See §1.4 for the algorithm and §5.3 for the mastery-tracking shape. **Wave 9 SHIP (Option A per-vowel sub-mastery):** the predicate is a hard runtime gate; the engine cannot under any session cadence introduce `/e/` adjacent to a non-mastered `/i/`. **Wave 7 historical posture (Option B composite, now superseded):** the cross-session ban was approximated by the §1.4 algorithm step 4 (planner skipped `/ɛ/`-targeted sessions until `/ɪ/` had separation time) plus the directive-level ≥2-session-gap rule. Wave 9 converts that approximation into the enforced predicate.

The acoustic-similarity ban is the load-bearing pedagogical rule for this tier — same way "≤2 b/d/p/q targets per session" is the load-bearing rule for A1 letter-names. Under Wave 9 it is enforced at the engine level rather than at the directive level.

### 1.3. Pool composition rule — 8 problems per session

Each Word Song letter-sounds session emits **8 problems**, matching the Math + CVC-tier + A1 session shape. Composition depends on **which vowel is current-target** for that session (the planner derives this from `progress.literacy.letterSoundsVowelStates` per §1.4 algorithm; falls back to all-`'intro'` when the field is absent per §1.4 fallback rule):
...
The planner picks the current-target vowel by reading **per-vowel sub-mastery state** directly from `progress.literacy.letterSoundsVowelStates`. The four short vowels each carry one of three states (the `unintroduced | practicing | mastered` triple mirrors the tier-level mastery state machine, but is tracked sub-skill-scoped to the vowel rather than to the SkillNode):

- `unintroduced` — vowel hasn't appeared as a target yet
- `practicing` — vowel has appeared as a target in ≥1 session and has not yet hit the per-vowel sub-mastery threshold
- `mastered` — vowel has hit the per-vowel sub-mastery threshold (see §5.3)

**Derivation algorithm (Wave 9 SHIP — reads sub-mastery state directly):**

1. Walk vowels in locked order: `/ɒ/, /ʌ/, /ɪ/, /ɛ/`.
2. **Read each vowel's sub-state directly from `letterSoundsVowelStates`.** The first vowel in `practicing` state is current-target. (This is the common case: Marian is mid-stream on a vowel and the planner just continues practising it.)
3. **If no vowel is `practicing`, the first `unintroduced` vowel in the sequence becomes current-target** — emission begins, and the W9.3 mastery-rule step transitions that vowel to `practicing` on session-end. The state read is direct; the planner makes no inference about "where Marian is" from history.
4. **Redundant safety: `/ɛ/` adjacency guard.** If step 2 or step 3 would pick `/ɛ/` while `letterSoundsVowelStates['/ɪ/'] !== 'mastered'`, abort and skip to the next mastered vowel for review-mode emission. Under Option A this branch is **unreachable in normal play**: the locked sequence `/ɒ/, /ʌ/, /ɪ/, /ɛ/` plus the per-vowel sub-state machine guarantees `/ɛ/` cannot be selected ahead of `/ɪ/` mastery (steps 2 and 3 walk in order and stop at the first `practicing`/`unintroduced` vowel they find). The guard is retained as belt-and-braces against a future progress-state mutation bug or a manual debug seed that skips `/ɪ/`. Implements the cross-session ban from §1.2 #2.

This decision is **MORE COMPLEX** than the simple "pick the focused vowel" rule used by CVC short-vowel tiers (which read focus directly from `effectiveFocusNode`). Letter-sounds is a single tier carrying multiple vowel sub-states. The added complexity is justified by the pedagogy — see §5.3 for the mastery rule and the Wave 7 → Wave 9 transition.

**Fallback rule — `letterSoundsVowelStates` field absent on the loaded `progress`:** when the field is missing (legacy localStorage blob written before Wave 9 / forward-compat read-path remap migration per `progress-and-persistence.md` §"Read-path remap migrations"), **the planner defaults all four vowels to `'intro'` (i.e. the `unintroduced` sub-state) and introduces `/ɒ/` first** per step 3 of the algorithm. This is the safe default — Marian's first vowel-to-master is `/o/`, so an empty/absent state is observationally equivalent to a brand-new arrival at this tier. The full migration path (W9.2) infers the field on first read from existing tier-level state via a read-path defaulter analogous to `withDefaultedSkillLevels` (per `progress-and-persistence.md` §"Sync points when widening `SkillNode`"); the all-`'intro'` fallback is what the defaulter produces for greenfield Marian. See W9.2 ticket for the read-path defaulter shape.
...
### 5.3. Promotion to next tier — Option A per-vowel sub-mastery (Wave 9 SHIP) + Option B fallback for back-compat

**Wave 9 SHIP target: Option A per-vowel sub-mastery.** The cross-session `/i/ → /e/` adjacency ban (§1.2 #2) is converted from a Wave-7 directive-level approximation into a **hard runtime predicate** on `letterSoundsVowelStates['/ɪ/'] === 'mastered'`. Per-vowel sub-state surfaces stuck vowels to the engine + orchestrator (per Kyle §5.4 risk mitigation), and the picker walks past `letter-sounds` only when all four short vowels have hit sub-mastery.

**Wave 7 historical context.** Wave 7 shipped Option B (composite-tier mastery) as the right-sized Wave 7 ship; Q4 (resolved 2026-05-23 via team rec, Thomas accepted per `[[feedback_no_sponsor_as_expert]]`) explicitly deferred Option A to a follow-up wave. Wave 9 (parent ticket `86c9y5d9x`, plan `design/wave-9-plan.md`) is that follow-up. Both options remain documented below — Option B's discussion is preserved because the W9.2 read-path defaulter and W9.3 composite-tier fallback still consume the Option B semantics when `letterSoundsVowelStates` is absent.

**Option A — per-vowel sub-mastery within `letter-sounds` (Wave 9 SHIP).**

- Each of `/ɒ/, /ʌ/, /ɪ/, /ɛ/` carries its own sub-mastery state (`unintroduced | practicing | mastered`) in `progress.literacy.letterSoundsVowelStates`.
- A vowel masters when its target-emission accuracy hits the per-track threshold (90% per §7 Q6 resolution; see §6) across the last 3 sessions in which it was current-target.
...
- The current-target derivation (§1.4) reads sub-mastery state directly — no turn-order approximation.
- The `/ɪ → /ɛ/` adjacency ban (§1.2 #2) is enforced as the runtime predicate `letterSoundsVowelStates['/ɪ/'] === 'mastered'` — the engine refuses to introduce `/ɛ/` as current-target until that predicate holds, regardless of session cadence.

**Pros:** matches pedagogy precisely; honours the `/i/` → `/e/` sequencing ban from §1.2 #2 as a hard runtime gate (don't introduce `/e/` until `/i/` sub-mastered); gives Marian credit per-vowel as she progresses; surfaces a stuck `/ɪ/` (or any vowel) explicitly to the engine + orchestrator. **Cons:** requires `progress` doc shape change (add `progress.literacy.letterSoundsVowelStates: Record<Vowel, MasteryState>`), `defaults.ts` widening, `cloudSync.ts` parity test, read-path defaulter, and a new `mastery.ts` branch. Wave 9 absorbs this cost across W9.2 (Kevin types + read-path migration), W9.3 (Devon mastery + bake-metadata), and W9.4 (Kevin planner predicate).

**Option B — composite mastery on the whole pool (Wave 7 SHIP, retained as fallback in Wave 9).**

- Standard 90% across 3 sessions — same rule as every other tier (uses MARIAN's existing M3 rule unchanged; no `mastery.ts` change needed at the tier level).
...
- The `/i/` → `/e/` in-session adjacency ban is enforced at the per-session level only (§1.2 hard constraint #1). The cross-session ban (§1.2 hard constraint #2) is approximated by the §1.4 algorithm step 4 (planner skips `/ɛ/`-targeted sessions until `/ɪ/` has had separation time) and by the directive-level rule that an `/ɛ/`-targeted session must follow an `/ɪ/`-targeted session by ≥2 intervening sessions.

**Wave 9 fallback rule: Option B fires when `letterSoundsVowelStates` is missing from the loaded `progress` document.** This is the forward-compat read path per `progress-and-persistence.md` §"Read-path remap migrations" — older localStorage blobs written before Wave 9's W9.2 widening do not carry the field. The W9.2 read-path defaulter fills the field on first read (greenfield → all-`'intro'`; mid-tier → migration-inferred from existing tier-level state) so the absent-field branch is observable only briefly during a single load. While the field is absent, the W9.3 mastery rule uses Option B's composite 90/3 over `skillFocus.includes('letter-sounds')` history entries; once the defaulter has run and the field is present, Option A takes over on the same session-end call. Both options share the same per-track 90/3 threshold (§7 Q6 resolution).

**Implementation sequencing (Wave 9):** W9.2 (Kevin) lands the field + read-path defaulter + cloudSync parity test. W9.3 (Devon) lands the per-vowel mastery branch + `bakeMetadata.perVowelTrackingActive` flag in `letter-sounds.json` + composite-tier fallback path. W9.4 (Kevin) lands the planner predicate + canon/cache bypass extension. W9.5 (Jessica) lands the failing-first E2E covering all five per-vowel transitions + composite fallback. See `design/wave-9-plan.md` § Sequencing for the dispatch chain.

### 5.4. Realistic mastery timeline for Marian

Given Marian's mastered-consonants + mastered-/æ/ + novel-/o,u,i,e/ starting state (Wave 9 / Option A sub-state transitions in-line):

- **Session 1 (current-target /ɒ/)**: 7-8 of 8 correct on first tap. The 2-3 /ɒ/-target items are the only items at real risk — Marian must form a brand-new sound→letter mapping. Plausibly 1-2 first-tap-wrong on /ɒ/ items, recoverable via hint slot. Result: `skillLevels['letter-sounds']` flips `intro → practicing`; `letterSoundsVowelStates['/ɒ/']` flips `unintroduced → practicing` on session-end.
- **Sessions 2-3 (current-target /ɒ/, then /ʌ/)**: continued /ɒ/ practice + first /ʌ/ session. Once `letterSoundsVowelStates['/ɒ/']` hits `mastered` (per-vowel 90/3 across `/ɒ/`-target sessions), the picker advances to `/ʌ/` per §1.4 step 2; `letterSoundsVowelStates['/ʌ/']` flips `unintroduced → practicing` on the first /ʌ/-target session-end. `/ʌ/` is the predicted high-risk vowel per `phonics-sequence-marian.md` §Q1 (Tagalog L1 interference — Marian renders `/u/` as long-oo). Expect 5-6/8 on first /ʌ/-target session; recovers over sessions 4-5.
- **Sessions 4-6 (continued /ʌ/ + first /ɪ/)**: `letterSoundsVowelStates['/ʌ/']` masters; `letterSoundsVowelStates['/ɪ/']` transitions `unintroduced → practicing`. `/ɪ/` is Marian's known weakness; expect 4-5/8 on first /ɪ/-target session. May need 3-4 sessions to consolidate. While `letterSoundsVowelStates['/ɪ/'] !== 'mastered'`, the §1.2 #2 runtime predicate keeps `/ɛ/` off the current-target slot — even if `/ɪ/` sticks, `/ɛ/` will not surface.
- **Sessions 7-10 (continued /ɪ/, then /ɛ/)**: once `letterSoundsVowelStates['/ɪ/']` hits `mastered`, the runtime predicate clears and `letterSoundsVowelStates['/ɛ/']` transitions `unintroduced → practicing` on the first /ɛ/-target session. Expect 5-6/8 on first /ɛ/-target session.
- **Sessions ~10-15**: `letterSoundsVowelStates['/ɛ/']` masters → all four short vowels at `mastered` → tier-composite gate fires → `skillLevels['letter-sounds']` flips `practicing → mastered` → picker walks to `blending-cv`.

**Mastery threshold for the tier: 90/3 (standard word-song threshold), resolved §7 Q6.** Applied per-vowel under Option A (each vowel masters when its target-emission accuracy hits 90/3 across the last 3 sessions in which it was current-target). The threshold is **not lowered** for letter-sounds despite earlier draft framing that suggested an 80% novel-acquisition allowance. Three reasons (load-bearing for the resolution):

1. **Pickering PMC5843573** — 90% over-learning is the durable plateau the project locked the word-song default to (see `progress-and-persistence.md` §"Per-track defaults" lines 394-397). 95% buys practice time without measurable benefit; the project explicitly chose 90/3 over 95/3 for that reason. A per-tier lower override would violate that locked anchor without comparable evidence.
2. **14 of 16 letter-sound mappings in the §1.1 pool are review for Marian** — consonants are mastered per CLAUDE.md current-levels + `phonics-sequence-marian.md` §Q2 ("consonants mostly transfer" from Tagalog L1), and short-/æ/ is mastered. Only 2 of 16 mappings are novel per session (the current-target vowel + the mastered anchor vowel that's now review). The "novel acquisition needs a lower threshold" framing is overstated when 14/16 items are review.
3. **The §1.3 composition cap of 2-3 novel-target emissions per session already protects against `/ɪ/` underconsolidation.** A session can have at most 3 novel-vowel-target items out of 8 — if Marian gets 2/3 of those wrong she's still at 75% session accuracy, comfortably above the failure floor, and the §1.4 algorithm guarantees `/ɪ/` is revisited the following session (it stays at `practicing` until 90/3 hits, so step 2 keeps picking it). The 90/3 rule with the 2-3 cap composes correctly; a lower per-tier threshold would have been belt-and-braces, not necessary protection.

**Risk: /i/ or /e/ sticks below mastery threshold indefinitely.** Mitigation under Wave 9 (Option A): per-vowel sub-mastery state surfaces a stuck `/ɪ/` (or any vowel) explicitly without blocking the rest of the tier — `letterSoundsVowelStates['/ɪ/']` stays at `'practicing'`, the §1.2 #2 runtime predicate keeps `/ɛ/` off-current-target, and the orchestrator can dispatch a targeted intervention if a vowel's sub-state sticks across many sessions. Mitigation under the Option B fallback path (field absent): the composite 90/3 measures whole-tier accuracy, the 2-3 novel-emission cap keeps any single vowel's contribution bounded, and the §1.4 step 4 redundant-safety guard prevents `/ɛ/` introduction adjacent to a struggling `/ɪ/`.

---

## 6. Mastery rule (summary)

**Wave 9 SHIP — Option A per-vowel sub-mastery under 90/3 (cross-day-deduped).** Each of `/ɒ/, /ʌ/, /ɪ/, /ɛ/` carries its own `unintroduced | practicing | mastered` state in `progress.literacy.letterSoundsVowelStates`. A vowel masters when its target-emission accuracy hits **90% across the last 3 sessions in which it was current-target** (the same per-track threshold the rest of the word-song tree uses, per §7 Q6 resolution — no per-tier override). The TIER (`letter-sounds`) masters when **ALL FOUR vowels are sub-mastered**, at which point `skillLevels['letter-sounds']` flips `practicing → mastered` and the picker walks to `blending-cv`.

The `/i/ → /e/` cross-session adjacency ban (§1.2 #2) is enforced as the runtime predicate `letterSoundsVowelStates['/ɪ/'] === 'mastered'` — the engine refuses to introduce `/ɛ/` as current-target until that predicate holds.

**Composite-tier fallback (Option B) — fires ONLY when `letterSoundsVowelStates` is absent from the loaded `progress` document.** Forward-compat read path for legacy localStorage blobs written before Wave 9's W9.2 widening. While the field is absent, the standard word-song 90/3 cross-day-deduped rule runs over `skillFocus.includes('letter-sounds')` history entries, and the §1.4 algorithm's redundant-safety guard (step 4) approximates the `/ɪ/ → /ɛ/` adjacency ban via session-turn-order plus the directive-level ≥2-session-gap rule. The W9.2 read-path defaulter fills the field on first read (greenfield → all-`'intro'`; mid-tier → migration-inferred from existing tier-level state), so the fallback branch is observable only briefly during a single load before Option A takes over. The 80% novel-acquisition allowance discussed in earlier drafts was withdrawn — see §5.4 for the three load-bearing reasons the standard 90/3 holds.

**Citation chain.** Q4 + Q6 resolved 2026-05-23 via team consultation (Dave + Kyle joint memo), Thomas accepted per `[[feedback_no_sponsor_as_expert]]`; see §7 for the full audit trail. Wave 9 ship target locked 2026-05-24 (sponsor decision — literacy continuation, accepting Wave 7 retro / Wave 8 sequencing); see `design/wave-9-plan.md` § TL;DR.

Implementation note: Wave 9 ships Option A as the runtime ship target across W9.2 (Kevin types + read-path defaulter), W9.3 (Devon `mastery.ts` per-vowel branch + `bakeMetadata.perVowelTrackingActive: true` on `letter-sounds.json`), W9.4 (Kevin planner predicate), W9.5 (Jessica failing-first E2E). Option B remains in `mastery.ts` as the documented fallback path; W9.3's regression-pin test asserts the fallback fires when the field is absent.

---

## 7. Open questions for sponsor

These are the decisions that needed Thomas's call before A6 (Dave's directive) was locked. **All adopt A1's defaults where applicable (see §0).** Two questions were originally surfaced here as genuine sponsor escalations from this spec — five other open-question candidates that surfaced during authoring were resolved as Devon/Dave impl calls or pre-locked precedent (see §8 appendix "Questions resolved during review" for the routing audit). Both Q4 and Q6 were **resolved 2026-05-23** via team consultation (Dave + Kyle joint memo) with Thomas accepting the team recommendation per `[[feedback_no_sponsor_as_expert]]` — specialist-domain calls (pedagogy / mastery shape / threshold calibration) are delegated to the team, with Thomas's lane reserved for strategic / scope / sequencing calls.

| #      | Status                                                                                                                                                                                                                                          | Resolved outcome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Q4** | **RESOLVED 2026-05-23** — team rec (Dave + Kyle joint memo), Thomas accepted per `[[feedback_no_sponsor_as_expert]]`. **Wave 9 follow-up (2026-05-24): sponsor accepted literacy continuation; Option A now SHIP via `design/wave-9-plan.md`.** | **Outcome: Option B — composite-tier mastery for Wave 7.** Uses MARIAN's existing M3 rule unchanged; no `mastery.ts` or `progress`-shape migration required. The `/ɪ/ → /ɛ/` cross-session adjacency ban was approximated via the Haiku directive's session-turn-order rule already encoded in §1.4 algorithm step 4 (planner skips `/ɛ/`-targeted sessions until `/ɪ/` has had separation time). **Option A (per-vowel `letterSoundsVowelStates`) is the Wave 9 SHIP** as a tighter follow-up that gates the adjacency ban on `letterSoundsVowelStates['/ɪ/'] === 'mastered'` rather than session turn order; Wave 9 plan locked 2026-05-24 (`design/wave-9-plan.md`). Wave 9 also retains Option B as the field-absent fallback path so legacy localStorage blobs round-trip cleanly until the W9.2 read-path defaulter has filled the field. **Rationale (original Wave 7 framing, retained for audit):** Wave 7 ship velocity mattered — Option B kept A7's surface to canon-bake + planner-directive + lint binding without forcing a Kevin/Devon progress-shape migration. The cross-session ban approximation via session-turn-order was pedagogically sufficient for the first run (Marian's vowel cadence is fixed cycling, so `/ɛ/` was naturally separated from non-mastered `/ɪ/` by the directive's ordering rule). Option A becomes the right shape now that Wave 7 telemetry confirms the basic plumbing works — at this point per-vowel granularity is worth the migration cost. **Citation:** §5.3 Option A body (Wave 9 SHIP) + §1.4 algorithm (reads sub-mastery state directly) + §1.2 constraint #2 (runtime predicate). |
| **Q6** | **RESOLVED 2026-05-23** — team rec (Dave + Kyle joint memo), Thomas accepted per `[[feedback_no_sponsor_as_expert]]`.                                                                                                                           | **Outcome: standard word-song 90/3 gate unchanged** — no per-tier threshold override. **Rationale (load-bearing):** (a) `progress-and-persistence.md` §"Per-track defaults" anchors the word-song 90/3 default to Pickering et al. PMC5843573 — 90% over-learning is the durable plateau, and the project explicitly chose 90/3 over 95/3 for that reason; (b) **14 of the 16 letter-sound mappings in the §1.1 pool are review-mode for Marian** (consonants mastered per CLAUDE.md diagnostic + `phonics-sequence-marian.md` §Q2 "consonants mostly transfer" + short-/æ/ mastered), so only 2/16 are novel per session — the "novel acquisition needs a lower threshold" framing in earlier §5.3/§5.4 draft was overstated; (c) the §1.3 composition cap of 2-3 novel-target-vowel emissions per session already protects against `/ɪ/` underconsolidation by limiting how much novel material lands in any single 8-problem run. **Citation:** `progress-and-persistence.md` §"Per-track defaults" lines 394-397 (Pickering anchor); CLAUDE.md current-levels table (consonants mastered); `phonics-sequence-marian.md` §Q2 (consonants transfer from Tagalog L1); §1.3 composition rule (2-3 cap on current-target vowel).                                                                                                                                                                                                                                                                                                                                                                                                               |
