...
The genuinely-new A5 decisions that need §7 escalation are: (i) read-line template phrasing for an isolated-phoneme prompt, (ii) SSML scope for phoneme utterances, (iii) per-session vowel-progression rule (which vowel(s) are eligible as targets based on Marian's mastery state), (iv) /i//e/ adjacency ban enforcement (in-session vs. cross-session), and (v) the mastery-progression decision for whether `letter-sounds` masters per-vowel or as one composite tier. See §7.
...
`api/_tts.ts:194-198` ships `PHONEME_OVERRIDES = { four: 'fɔːr' }` — a substitution registry that wraps whole-word matches at TTS render time. This existing infrastructure **is the wrong shape for letter-sounds** because:

- The override is keyed on **plain English words** (e.g. `four`). Letter-sounds needs to wrap **specific isolated-character mnemonics** (`m`, `mmm`, etc.).
- The override applies to **any utterance** that contains the word. Letter-sounds' wraps are utterance-template-specific; applying them universally would break other tiers (e.g. wrapping the letter `m` in every CVC utterance would mispronounce words like "math" or "moth").

**Recommended SSML integration architecture (for A6 Dave directive + A7 Devon bake):** the Haiku directive emits the utterance text **with SSML markup already inline** in the canon JSON's `utterances[].text` field. The browser-side renderer (`api/_session.ts` / `api/_tts.ts`) treats SSML-bearing text as opaque — it ships verbatim to Azure with the existing `<speak>` wrapper.

This requires verifying that `api/_tts.ts` does NOT escape `<` and `>` in user-provided text before sending to Azure (the existing `PHONEME_OVERRIDES` injection happens AFTER escaping; user-provided SSML in the text would be double-escaped). **Open as §7 Q1 for Devon to verify pre-bake.** If escaping is on the path, the architecture flips to: the canon JSON ships plain mnemonic text, and `api/_tts.ts` gains a tier-aware phoneme substitution table.

The first architecture (inline SSML in canon) is preferred because it scopes the SSML to the canon bake — no shared registry, no cross-tier collision risk. The second (substitution table) preserves the existing `PHONEME_OVERRIDES` pattern but requires `api/_tts.ts` to know which tier the utterance came from, which is a new coupling surface.
...
These are the decisions that need Thomas's call before A6 (Dave's directive) is locked. **All adopt A1's defaults where applicable (see §0).** The questions below are GENUINELY NEW for A5 — they did not arise in the letter-names tier.

| #      | Question                                                                                                                                                                                                                                                                                                                                                                               | Recommendation                                                                                                                                                               | Impact if flipped                                                                                                                                                                                                                                                 |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Q1** | **SSML emission architecture — inline-in-canon vs. tier-aware substitution table in `api/_tts.ts`?** Spec §2.4 recommends inline-SSML in the canon JSON (Haiku directive emits utterances with `<phoneme>` markup already present; `api/_tts.ts` ships verbatim to Azure). Devon needs to verify pre-bake that `api/_tts.ts` does NOT XML-escape user-provided text before Azure send. | Inline SSML in canon. Verify pre-bake.                                                                                                                                       | If escape-on-path is confirmed: spec flips to extending `PHONEME_OVERRIDES` with a tier-aware mode. Adds coupling between `api/_tts.ts` and the canon bake; reversible but more code.                                                                             |
| **Q2** | **Speech-ribbon caption — SSML strip-and-display or `displayText` companion field?** Spec §4.4 recommends a `stripSsml(text)` helper at render time; Devon may prefer adding a `displayText: string` companion field to the canon `utterance` shape.                                                                                                                                   | Helper at render time (no canon shape change).                                                                                                                               | If flipped to `displayText`: canon JSON size grows ~10-20% for SSML-bearing utterances; `wordSongSessionPlanFromWire` adapter widens to consume the new field. Reversible.                                                                                        |
| **Q3** | **`compositionLint` binding for letter-sounds — yes or no?** A7 ticket flags as optional. The §1.3 + §3.1 + §1.2 + §3.2 rules HAVE mechanical structure (current-target vowel count 2-3, gentle-window pure-consonant, /i//e/ exclusive, voiced/unvoiced pair traps). Spec adopts A1's Q6 default = YES.                                                                               | YES — bind a `TierLintBinding` for letter-sounds mirroring sub-to-10's shape (`RULE_IDENTITY+SPEC+LINT` triple-pin per `[[feedback_haiku_directive_sharpening]]` Pattern 7). | Adds 5-7 lint rules + ~12-18 unit tests to Devon's A7 PR; pre-disk catch on Haiku drift. Reversible (can drop the binding post-ship if false-positives).                                                                                                          |
...
| **Q5** | **Distractor scope of /i//e/ ban — MUST (no /i//e/ as distractors when the other is current-target) or SHOULD (allow on non-vowel-target problems)?** Spec §3.2 recommends SHOULD — looser distractor scope keeps the chip variety up without compromising the target-side ban.                                                                                                        | SHOULD (looser distractor scope). The hard constraint stays on TARGETS only.                                                                                                 | If flipped to MUST: distractor pool for /i/-target sessions shrinks; some valid trios become unreachable. Cosmetic constraint — would tighten the lint binding by ~2 rules.                                                                                       |
...
| **Q7** | **Voiced-th `/ð/` inclusion — confirm exclusion per spec §1.1 table?** The voiced-th phoneme is excluded from this tier on the same conservative basis as `digraphs-th-voiceless` excludes it (see that spec §1.4). Decision: exclude. Listed here for sponsor visibility, not for active debate.                                                                                      | Exclude. (Decision pre-locked by digraphs-th-voiceless precedent.)                                                                                                           | N/A — no flip expected. Listed for transparency only.                                                                                                                                                                                                             |
...
3. **`api/_tts.ts` SSML escaping verification (Q1) blocks A6 directive authoring.** If Devon confirms escape-on-path, the entire utterance template architecture in §2 flips. Recommendation: **Devon spike + verify BEFORE Dave starts A6 directive.** Small 1-hour spike — check if `<` and `>` from user-message-derived utterance text are escaped or preserved at the Azure-send boundary.
