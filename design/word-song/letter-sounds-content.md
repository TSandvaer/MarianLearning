...
2. **Cross-session, `/e/` introduction MUST wait until `/i/` has reached `practicing → mastered` status.** This is a tier-internal constraint that the planner cannot enforce alone (planner is session-scoped, not session-history-aware) — see §5.3 for how mastery-tracking handles this. **Wave 7 implementation (resolved §7 Q4 — Option B composite):** the cross-session ban is approximated by the §1.4 algorithm step 4 (planner skips `/ɛ/`-targeted sessions until `/ɪ/` has had separation time) plus the directive-level ≥2-session-gap rule. **Wave 8 follow-up (Option A per-vowel sub-mastery):** track per-vowel sub-mastery within `letter-sounds` and gate `/e/`-targeted sessions behind `/i/` sub-mastery as a hard runtime gate rather than an approximation.
...
### 5.3. Promotion to next tier — composite vs per-vowel mastery (resolved §7 Q4)

This was the **single biggest pedagogical question** for this tier. **Resolved 2026-05-23 (team rec, Thomas accepted per `[[feedback_no_sponsor_as_expert]]`): Wave 7 ships Option B (composite-tier mastery); Wave 8 follow-up implements Option A (per-vowel sub-mastery).** Both options remain documented below — Option A is load-bearing for the Wave 8 follow-up ticket, so its discussion stays intact.

**Option B — composite mastery on the whole pool (Wave 7 SHIP).**

- Standard 90% across 3 sessions — same rule as every other tier (uses MARIAN's existing M3 rule unchanged; no `mastery.ts` change needed).
...
**Pros:** zero `progress` doc change; ships against today's shape; A7 is a small canon-bake + planner directive PR. **Cons:** Marian could mass `/ɛ/` before `/ɪ/` is mastered — the cross-session ban is approximated, not enforced. **Why this is acceptable for Wave 7:** Marian's vowel cadence under Option B is fixed cycling (the planner walks the locked sequence `/ɒ/, /ʌ/, /ɪ/, /ɛ/`), and the §1.4 step 4 skip rule plus the ≥2-session-gap rule together ensure `/ɛ/` cannot land adjacent to a struggling `/ɪ/` in practice. The Wave 8 Option A migration tightens this from approximation to enforcement.

**Option A — per-vowel sub-mastery within `letter-sounds` (Wave 8 FOLLOW-UP).**
...
- The current-target derivation (§1.4) reads sub-mastery state directly rather than approximating via turn-order.

**Pros:** matches pedagogy precisely; honours the `/i/` → `/e/` sequencing ban from §1.2 #2 as a hard runtime gate (don't introduce `/e/` until `/i/` sub-mastered); gives Marian credit per-vowel as she progresses. **Cons:** requires `progress` doc shape change (add `progress.literacy.letterSoundsVowelStates: Record<Vowel, MasteryState>`), `defaults.ts` widening, `cloudSync.ts` migration (per `sibling-tier-checklist.md` partial coverage), and a new `mastery.ts` branch — Kevin/Devon migration cost roughly +1 week to implement and verify. This is the right long-term shape but not the right Wave 7 shape.

**Implementation sequencing (locked 2026-05-23):** A7 in Wave 7 ships Option B only (no `mastery.ts` change, no `progress` migration). The Wave 8 follow-up ticket — filed by the orchestrator alongside the §7 resolution — implements Option A and migrates any letter-sounds session history accumulated under Option B into the per-vowel sub-state shape on first read.

### 5.4. Realistic mastery timeline for Marian

Given Marian's mastered-consonants + mastered-/æ/ + novel-/o,u,i,e/ starting state:

- **Session 1 (current-target /ɒ/)**: 7-8 of 8 correct on first tap. The 2-3 /ɒ/-target items are the only items at real risk — Marian must form a brand-new sound→letter mapping. Plausibly 1-2 first-tap-wrong on /ɒ/ items, recoverable via hint slot. Result: tier flips `intro → practicing`; under Wave 7 (Option B), first-time emission cycle progresses; under Wave 8 (Option A), `/ɒ/` sub-state additionally flips to `practicing`.
...
- **Sessions 7-10 (continued /ɪ/, then /ɛ/)**: under Wave 7 (Option B), `/ɛ/` arrives after the fixed ≥2-session gap rule from §5.3; under Wave 8 (Option A), `/ɛ/` arrives only after `/ɪ/` sub-mastery. Expect 5-6/8 on first /ɛ/-target session.
- **Sessions ~10-15**: tier flips `practicing → mastered`. Picker walks to `blending-cv`.

**Mastery threshold for the tier: 90/3 (standard word-song threshold), resolved §7 Q6.** The threshold is **not lowered** for letter-sounds despite earlier draft framing that suggested an 80% novel-acquisition allowance. Three reasons (load-bearing for the resolution):
...
3. **The §1.3 composition cap of 2-3 novel-target emissions per session already protects against `/ɪ/` underconsolidation.** A session can have at most 3 novel-vowel-target items out of 8 — if Marian gets 2/3 of those wrong she's still at 75% session accuracy, comfortably above the failure floor, and the directive's vowel-turn-order rule guarantees `/ɪ/` is revisited the following session. The 90/3 rule with the 2-3 cap composes correctly; a lower per-tier threshold would have been belt-and-braces, not necessary protection.

**Risk: /i/ or /e/ sticks below mastery threshold indefinitely.** Mitigation under Wave 7 (Option B): the §1.4 step 4 skip rule prevents `/ɛ/` introduction adjacent to a struggling `/ɪ/`, so neither vowel can block tier-mastery in isolation while the other is still consolidating — the composite 90/3 measures whole-tier accuracy, and the 2-3 novel-emission cap keeps any single vowel's contribution bounded. Mitigation under Wave 8 (Option A): per-vowel sub-mastery state surfaces a stuck `/ɪ/` (or `/ɛ/`) explicitly without blocking the rest of the tier, and the orchestrator can dispatch a targeted intervention if a vowel's sub-state sticks across many sessions.

---

## 6. Mastery rule (summary)

**Wave 7 (RESOLVED 2026-05-23): Option B composite — standard 90/3 cross-day-deduped rule** (no per-tier override). The cross-session `/i/` → `/e/` adjacency ban is approximated by the Haiku directive's session-turn-order rule (§1.4 step 4 + the ≥2-session-gap directive rule). Resolution per team consultation (Dave + Kyle joint memo), Thomas accepted per `[[feedback_no_sponsor_as_expert]]`. See §7 Q4 + Q6 for the full citation chain.

**Wave 8 follow-up: Option A per-vowel sub-mastery** under the same 90/3 threshold per vowel (the 80% novel-acquisition allowance discussed in earlier drafts was withdrawn — see §5.4 for the three load-bearing reasons the standard threshold holds). The Wave 8 ticket will add `progress.literacy.letterSoundsVowelStates` and migrate the §1.4 algorithm to read sub-mastery state directly instead of approximating via turn-order.

Implementation note: A7 ships Option B (no `mastery.ts` change, no `progress`-shape migration). A Wave 8 follow-up ticket has been filed by the orchestrator alongside this resolution; the ticket should reference §5.3 Option A and §1.4 step 4 for the migration target.
...
| ------ | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Q4** | **RESOLVED 2026-05-23** — team rec (Dave + Kyle joint memo), Thomas accepted per `[[feedback_no_sponsor_as_expert]]`. | **Outcome: Option B — composite-tier mastery for Wave 7.** Uses MARIAN's existing M3 rule unchanged; no `mastery.ts` or `progress`-shape migration required. The `/ɪ/ → /ɛ/` cross-session adjacency ban is approximated via the Haiku directive's session-turn-order rule already encoded in §1.4 algorithm step 4 (planner skips `/ɛ/`-targeted sessions until `/ɪ/` has had separation time). **Option A (per-vowel `letterSoundsVowelStates`) is deferred to Wave 8** as a tighter follow-up that will gate the adjacency ban on `/ɪ/` sub-mastery rather than session turn order. A Wave 8 follow-up ticket is filed by the orchestrator to track the Option A migration. **Rationale:** Wave 7 ship velocity matters — Option B keeps A7's surface to canon-bake + planner-directive + lint binding without forcing a Kevin/Devon progress-shape migration. The cross-session ban approximation via session-turn-order is pedagogically sufficient for the first run (Marian's vowel cadence is fixed cycling, so `/ɛ/` is naturally separated from non-mastered `/ɪ/` by the directive's ordering rule). Option A becomes the right shape once Wave 7 telemetry confirms the basic plumbing works — at that point per-vowel granularity is worth the migration cost. **Citation:** §5.3 Option B body + §1.4 step 4 (algorithm carries the adjacency-ban approximation). |
