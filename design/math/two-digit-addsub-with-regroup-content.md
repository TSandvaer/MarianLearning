...
**Status:** SPEC — draft for Thomas review. Implementation blocked on this PR merging. Kevin/Devon pick up impl after spec approval. **Dave's Wave-5 research note (`design/research/wave-5-borrow-carry-error-patterns.md`) had NOT shipped at the time this spec was authored**; §2 Distractor classes contains placeholders citing `<pending Dave research>` that must be filled before the Kevin/Devon implementation waves dispatch. The §2 mechanical specifications are author-defensible from existing precedent (Wave 4 Class 2 column-cross + Class 3 phantom-borrow) but the pedagogical-fit citation gate per `[[feedback_distractor_class_pedagogical_gates_mechanical]]` requires Dave's note before merge of any implementation PR.

**Ticket:** `86c9xwk74` (parent epic `86c9xwjtr`).

**Authority chain.**

- **Primary pedagogical authority (THIS TIER):** `<pending: design/research/wave-5-borrow-carry-error-patterns.md>` — Dave's research note on the borrow + carry error pattern landscape. Expected to consume: Brown-VanLehn (1980) bug catalog (esp. **Smaller-From-Larger** SFL, **Borrow-No-Decrement**, **Stops-Borrow-At-Zero**); Fuson (1990, 1997) for the carry-side "concatenated-with-carry-suppression" pattern; Geary (2004) on visual-spatial errors in multi-column work; NMAP (2008) for the sequencing recommendation. Until this note ships, §2's pedagogical-fit citations are placeholders. **Author note:** much of the Brown-VanLehn material was previewed in Dave's Wave-4 research note `design/research/two-digit-addsub-error-patterns.md` §1 (SFL bug catalog, "phantom borrow over-generalisation," Borrow-No-Decrement) — Wave 5 IS the tier where those error patterns become primary (rather than the diagnostic-instrument-only role they played in Wave 4).
- Predecessor research: [`design/research/two-digit-addsub-error-patterns.md`](../research/two-digit-addsub-error-patterns.md) (Dave, 2026-05-21, PR #285-adjacent) — explicitly framed the no-regroup tier as a *gate* for the regrouping tier ("phantom-borrow over-generalisation," "Borrow-No-Decrement," "SFL bug") with the structural false-positive correction lever shipped at Wave 4. Wave 5 is the natural successor: the bugs that were *diagnostic instruments* at Wave 4 (Class 2 column-cross, Class 3 phantom-borrow) become *primary error modes* at Wave 5 because the underlying arithmetic procedure (the regrouping step itself) is now the conceptual surface being taught.
...
Per Dave's Wave-4 research §1 (Brown-VanLehn bug catalog), the regrouping procedure has the highest documented per-fact error rate of any pre-multiplication arithmetic skill. This tier is **slow** by design — accuracy-only OUT-gate would be too lenient. The diagnostic-aware OUT gate framework from Wave 4 (`perProblemDistractorClass`) carries forward, with the Class assignments inverted (Class 2/3 are now the *expected* taps for a child who hasn't acquired the procedure; gate criterion is *reduction* of Class 2/3 taps over sessions, not blanket rejection).
...
**Pedagogical fit:** <pending Dave Wave-5 research § Class 0 confirmation — same posture as Wave 4 expected>.
...
**Pedagogical fit:** <pending Dave Wave-5 research § Class 1 — expected to confirm: counting-drift-inside-regroup is the highest-frequency per-fact error class per Brown-VanLehn §3 bug-frequency table>.
...
> **DEGENERATE-CASE FLAG.** Several Wave-5 pool facts produce palindromic-units answers because the carry frequently lands on a multiple-of-11 result (`33`, `22`, `44`, `55`). Lint should assert the degenerate-fraction ≤ 25% across the pool (looser than Wave 4's ≤ 10%). The downgrade-chain handles the rest. <pending Dave Wave-5 research § Class 2 fit-on-regroup — confirm the swap-distractor is pedagogically meaningful when the underlying error mode is SFL rather than the concatenated-single-digit-processing pattern from Wave 4>.

**Pedagogical fit:** <pending Dave Wave-5 research § Class 2 — expected to confirm: column-swap maps to Brown-VanLehn SFL bug under regroup, where the child swaps the column subtraction direction to "always smaller-from-larger"; the trap chip mirrors that error>.
...
>
> **Spec recommendation:** at Wave 5, the Class 3 helper produces EITHER `correct − 10` OR `correct + 10`, alternating across the session. <pending Dave Wave-5 research § Class 3 — confirm the alternation strategy + select which is the higher-frequency bug>.

**Pedagogical fit:** <pending Dave Wave-5 research § Class 3 — confirm Brown-VanLehn bug pedigree and frequency for Borrow-No-Decrement at Wave 5 specifically (vs Wave 4 phantom-borrow over-generalisation)>.
...
**Pedagogical fit:** <pending Dave Wave-5 research § Class B — confirm decade-anchor lock as the documented `round-ten-cross-down` failure mode>.
...
export const TWO_DIGIT_ADDSUB_WITH_REGROUP_RULES: TwoDigitAddsubWithRegroupRulesConfig = {
  ...
  opMix: {
    addRange: [5, 6],
    subRange: [2, 3],
    forbiddenMixes: [[8, 0], [7, 1], [4, 4], [3, 5]],
  },
...
Add `two-digit-addsub-with-regroup` to the binding resolution at `scripts/compositionLint.ts:3015+`:

```ts
if (focusNode === 'two-digit-addsub-with-regroup') {
  return { tier: 'two-digit-addsub-with-regroup', config: TWO_DIGIT_ADDSUB_WITH_REGROUP_RULES }
}
```
...
### 6.2 Dave's Wave 5 research note (PENDING)

- `<pending: design/research/wave-5-borrow-carry-error-patterns.md>` — expected to consume: Brown-VanLehn (1980) bug catalog (SFL, Borrow-No-Decrement, Stops-Borrow-At-Zero); Fuson (1990, 1997) for the "concatenated-with-carry-suppression" pattern; Geary (2004) on visual-spatial errors in multi-column work; NMAP (2008) for sequencing. **§2 distractor classes pedagogical-fit citations all carry the `<pending Dave Wave-5 research § …>` placeholder until this note ships.** The §2 mechanical specifications are author-defensible from Wave-4 precedent; the pedagogical-fit citations are not. Per `[[feedback_distractor_class_pedagogical_gates_mechanical]]`, pedagogical fit gates mechanical fit — implementation waves (Kevin Wave 1b / Wave 2; Devon Wave 3) do NOT dispatch until Dave's note lands and the placeholders are resolved.
