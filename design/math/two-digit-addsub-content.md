...
- `round-ten-anchor` — the first operand ends in zero (e.g. `20 + 3`, `40 − 5`). Tens digit unchanged by operation; child reads "twenty" and appends/removes a one-digit count. The "trivial" entry point for this tier; high salience for Haiku's saturation-prior (across many bakes Haiku gravitates toward `20+, 30+, 40+`-shape facts as the easiest representational instance of the tier — see §1.4). **Capped tight.**
...
2. **Caps `round-ten-anchor` at 3 pool entries** (`20+3, 30+5, 40+2`) with a session cap of 1 (§2.2). Haiku's uncapped prior empirically saturates this category (see §1.4); the cap pins it to one-eighth of the session.
...
The empirical failure mode this cap prevents: **Haiku's uncapped saturation-prior**. Across multiple bakes of this tier with under-constrained directives, Haiku gravitates toward round-ten anchors (`20+3, 30+5, 40+2`) — the round-ten facts are the easiest representational instance of the tier (the units operation is trivial and the tens digit is preserved by inspection rather than by reasoning). Left unguarded, a single 8-problem bake can land 2 or 3 round-ten facts, crowding out the near-boundary-no-cross learning target. This is **identical in structure to the add-to-20 doubles-prior** — Haiku gravitates to the easiest representational instance of the tier when given an under-constrained directive.

> **Empirical-claim discipline (per `planner-and-canon.md` § "Directive-prose canon-state claims must be empirically verified", PR #293).** This rationale is intentionally framed around the **saturation-prior pattern observable across many bakes**, not around any single canon snapshot. A per-snapshot claim ("the current canon ships X round-ten facts") drifts every time the canon is re-baked, but the cap's underlying justification — Haiku's prior — is the durable, verifiable failure mode the cap prevents. The cap value (`AT MOST 1`) and the FORBIDDEN-pair negative anchors are unchanged; only the descriptive framing is.
...
| -------------------------- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `round-ten-anchor`         | 1   | Pool has 3 round-ten-anchor facts (`20+3, 30+5, 40+2`); cap at 1 holds the category to 1-of-8 (12.5% of the session) against Haiku's uncapped saturation-prior toward round-ten facts (left unguarded, bakes can land 2-3 round-ten anchors in a single 8-problem session). **Round-ten-prior correction lever.** Marian retrieves these trivially; they're representational filler, not learning target.                                                       |
...
| `read` (−)   | `"<minuend> minus <subtrahend>. How many are left?"`                              | `"Forty-eight minus seven. How many are left?"`                  | "minus" framing (NOT "take away"). The trailing phrase is `"How many are left?"` (NOT `"How many?"`) — the browser parser uses the trailing phrase to discriminate `-` from `+` (per `planner-and-canon.md` § "Sibling failure mode — Planner directive vs canon read-line template divergence"; PR #292 Wave 2 prereq fold-in for ticket 86c9xa817). The "minus" template is internalised by sub-to-X mastery. |
...
- [ ] **AC9**: Canon JSON `public/canon/math/level-1/two-digit-addsub.json` re-baked and committed. Canon-lint passes (ASCII-7, no slash-IPA, no angle tags, composition rules satisfied). File parses via `isSessionStartResponse`. **Round-ten-anchor count in the canon is ≤ 1** (the empirical proof the round-ten-prior correction landed against Haiku's saturation-prior toward round-ten facts). **Op-mix is in [5+/3-, 6+/2-]**.
...
- `public/canon/math/level-1/two-digit-addsub.json` — re-baked canon under the round-ten-anchor cap (per §1.4 / §2.3 saturation-prior framing); supersedes any prior unguarded bake.
