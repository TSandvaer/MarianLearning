...
- **Melody entrance (Wake state, on screen mount):** slides in from off-screen bottom-left.
  ...
  ```
  Total entrance ~700ms. Spring settles without bounce-past (damping 22 keeps it calm, not cartoonish). She lands in idle pose and **breathes**: `scale: [1, 1.015, 1]` over 2.4s, `repeat: Infinity`, `ease: "easeInOut"`. Subtle — reads as "alive," not "animated."
...
- **Wake re-prompt (no tap for 12s):** Melody plays a small "wave" gesture — ear-wiggle sprite swap for 600ms, then back to idle. **No TTS** (still locked). Ring continues pulsing. This is the *only* re-prompt; if no tap by 30s the screen sits indefinitely without further prompts. Rationale: an 8-year-old won't be punished for taking time, and we explicitly don't want a nag loop. The 12s cue exists because if Marian thinks the screen is frozen she'll close the app.
...
- `melody-happy.png` — ear-wiggle pose (ears slightly up/angled). Same dims. ~80 KB. **Reused for the Wake re-prompt wave at 12s.**
...
- [ ] **Wake re-prompt:** at 12s of no tap, Melody plays a single ear-wiggle wave (sprite swap to `melody-happy` for 600ms, then back). Triggers exactly once. No TTS during this re-prompt
